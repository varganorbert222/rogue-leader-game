...
import { normalizeAlphaCutoff } from './apply/blend-mode';
import { normalizeSubEmitters } from './sub-emitters-normalize';
import type { ParticleMeshSettings, ParticleSystemEditable } from './types';
...
export function normalizeParticleSystem(
  ...
  const merged: ParticleSystemEditable = {
    ...
    renderMode: system.renderMode === 'mesh' ? 'mesh' : 'billboard',
    mesh: normalizeMeshSettings(defaults.mesh, system.mesh),
    subEmitters: normalizeSubEmitters(system.subEmitters),
    alphaCutoff: normalizeAlphaCutoff(system.alphaCutoff),
  };
...
function normalizeMeshSettings(
  defaults: ParticleMeshSettings,
  patch?: Partial<ParticleMeshSettings>,
): ParticleMeshSettings {
  return {
    glbUrl: typeof patch?.glbUrl === 'string' ? patch.glbUrl : defaults.glbUrl,
    uniformScale:
      typeof patch?.uniformScale === 'number' && patch.uniformScale > 0
        ? patch.uniformScale
        : defaults.uniformScale,
    randomRotation: patch?.randomRotation ?? defaults.randomRotation,
  };
}
