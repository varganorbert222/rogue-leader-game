...
import { missionWaveCount, missionWaves } from "./mission-waves";
import type { MissionConfig, MissionWave } from "./mission-types";

export interface MissionWaveState {
  wavesSpawned: number;
  waveTimer: number;
  waveSpawnInProgress: boolean;
}

export interface MissionWaveSpawnContext {
  world: ActorWorld;
...
export function createInitialWaveState(config: MissionConfig): MissionWaveState {
  return {
    wavesSpawned: 0,
...
export function updateMissionWaves(
  state: MissionWaveState,
  dt: number,
  config: MissionConfig,
  npcCount: number,
  spawnCtx: MissionWaveSpawnContext,
): void {
  ...
  spawnMissionWave(nextWave, spawnCtx);
  state.wavesSpawned++;
...
export function spawnMissionWave(
  wave: MissionWave,
  ctx: MissionWaveSpawnContext,
): void {
